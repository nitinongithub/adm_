<?php	
	...
	if(mysql_num_rows($result) > 0){
		//$qryReg="SELECT * FROM fee where regID ='" . $regno . "' AND Amount=2000 AND feetype='Registration'";
		$qryReg="SELECT * FROM fee where regID ='" . $regno . "'";
...
<center>
	...
	<?PHP if(mysql_num_rows($result) >0){?>
        ...
        <table border="0" cellpadding="0" cellspacing="0" width="700">
			...
			<td align="left" class="tdStyle2" height="25">&nbsp;Registration No. <span style="font-weight:bold; font-family:Arial, Helvetica, sans-serif; color:#000; font-size:18px"><?PHP echo $regno;?></span>
	<?PHP $qry_temp="SELECT * FROM stud_personal where id='" . $regno . "'";
	             $result_temp = mysql_query($qry_temp) or die(mysql_error()); ?>
                  <?PHP while($row_temp = mysql_fetch_array($result_temp)){?>
                     Date: <?PHP echo $row_temp['regDate'];?></td>
                   <?PHP } ?>
...
<?PHP
            ...
            <tr>
				...
				<td bgcolor="<?PHP echo $clrJi;?>" align="right" class="tdStyle2" width="400">
				<?PHP if(mysql_num_rows($fresult) > 0){?>
                        <?PHP $rowAdmission = mysql_fetch_array($fresult) or die(mysql_error());?>
                        <font color="#ffffff"><b>Admission taken in <font color="#ffff00"><?PHP echo $rowAdmission['courseID'];?> </font>Course </b>on <?PHP echo $rowAdmission['admissiondate'];?> </font>
                <?PHP }else{?>
