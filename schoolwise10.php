...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <body>
        <center>
                <H3><font color="#000000">Total High-School Students from different School in Year <?php echo date('Y'); ?></font></H3>
                <H3 id="totalm_f"></H3>
                <div style="overflow: auto; width: 700px; height: auto">
                    <table border="0" cellpadding="0" cellspacing="0" class="sortable" id="hovertable" width="500" style="font-size: 20px">
                        <tr>
                            ...
                            <?php
								$tot_count = mysql_query("select count(*) as count_ from z1_view_student_qualifiation_10");
								$tc_ = mysql_fetch_array($tot_count);
								if($tc_['count_'] != 0){
									$total_ = $tc_['count_'];
								}
                                $qry = "SELECT `instname`, count(`instname`) as count FROM `z1_view_student_qualifiation_10` group by `instname`";
